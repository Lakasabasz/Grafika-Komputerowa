...
void setMaterial(){
    glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, materials[0]);
    glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, &materials[0][8]);
    glMaterialf(GL_FRONT_AND_BACK, GL_SHININESS, materials[0][12]);
}
...
void initGL() {
    ...
    glEnable(GL_LIGHT0);
    glEnable(GL_LIGHT1);
    float lpos[]{8.0f, 8.0f, 0.0f, 0.0f};
    float ambient[]{.2, .5, .2};
    float diffuse[]{.8, .8, .8};
