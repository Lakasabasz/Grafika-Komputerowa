 ...
 */
void drawKorkociong(unsigned N){
    double x = -5;
    double angle = 2*M_PI*N;
    double stepAngle = angle / 1000.0; // Krok = pełna ilość obrotów podzielona na ilość punktów na całym rysunku
    double stepX = 10 / 1000.0;

    glColor3d(0.4, 0.4, 0.4);
    for(int i = 0; i<1000; i++){
        glPointSize(x+5.001);
        glBegin(GL_POINTS);
        glVertex3d(x, 2*sin(angle), 2*cos(angle));
        glEnd();
        x+=stepX;
        angle-=stepAngle;
    }
...
void drawPiramyd(unsigned N){
    for(unsigned i = 1; i<=N; i++){
        drawTriangle(2*cos((i-1)*2*M_PI/N), 2*sin((i-1)*2*M_PI/N), 2*cos(i*2*M_PI/N), 2*sin(i*2*M_PI/N));
    }
}
...
void display() {  // Display function will draw the image.
    ...
    std::cout << "Draw object " << objectNumber << std::endl;
...
void doKeyboard( unsigned char ch, int x, int y ) {
    std::cout << "Keyboard event: " << ch << " " << x << " " << y << std::endl;
    int redraw = 1;
    if ( ch == '1')
        objectNumber = 1;
    else if ( ch == '2')
