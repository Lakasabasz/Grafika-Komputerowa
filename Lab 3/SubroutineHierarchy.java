...
public class SubroutineHierarchy extends JPanel {
	...
	private void drawWorld(Graphics2D g2) {
		...
		rotatingCircle(g2, -1, .5, 1);
		rotatingCircle(g2, 1, 0, 1);
		bline(g2, -1, 0.5, 1, 0);
		triangle(g2, 0, 0.25, Color.blue);
		...
		rotatingCircle(g2, 1, 0, 1);
		bline(g2, -1, 0.5, 1, 0);
		triangle(g2, 0, 0.25, Color.MAGENTA.darker());
		
		g2.scale(0.8, 0.8);
		g2.translate(5.5, 0.5);
		rotatingCircle(g2, -1, .5, 1);
		rotatingCircle(g2, 1, 0, 1);
		bline(g2, -1, 0.5, 1, 0);
		triangle(g2, 0, 0.25, Color.GREEN.darker());
	...
	//private void 
	private void triangle(Graphics2D g2, double x, double y, Color c) {
		...
		g2.translate(x, y);
		g2.scale(0.2, 1);
		g2.fillPolygon(new Polygon(new int[] {-1, 0, 1}, new int[] {-2, 0, -2}, 3));
		g2.setStroke(saveStroke);
		g2.setColor(saveColor);
		g2.setTransform(saveTransform);
	}
	
	private void bline(Graphics2D g2, double x, double y, double x2, double y2){
		AffineTransform saveTransform = g2.getTransform();  // (It might be necessary to save/restore transform and color)
		Color saveColor = g2.getColor();
		Stroke saveStroke = g2.getStroke();
		g2.setStroke(new BasicStroke(0.15f));
	...
	private void rotatingCircle(Graphics2D g2, double x, double y, double r) {
		AffineTransform saveTransform = g2.getTransform();
		Color saveColor = g2.getColor();
		Point2D[] list = new Point2D[6];
		double animationOffset = frameNumber/20.0;
		for(int i = 0; i<6; i++) {
			list[i] = new Point2D.Double(
					(r/2)*Math.cos(animationOffset + (i*2*Math.PI/5)) + x,
					(r/2)*Math.sin(animationOffset + (i*2*Math.PI/5)) + y);
		}
		Point2D center = new  Point2D.Double(x, y);
		for(int i = 0; i<6; i++) g2.draw(new Line2D.Double(center, list[i]));
		for(int i = 1; i<6; i++) g2.draw(new Line2D.Double(list[i-1], list[i]));
		g2.setColor(saveColor);
		g2.setTransform(saveTransform);
	}
